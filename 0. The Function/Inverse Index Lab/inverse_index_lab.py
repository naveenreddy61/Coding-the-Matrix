...
def movie_review(name):
    ...
    """
    return ["See it!", "A gem!", "Idealogical Claptrap"][randint(0, 2)]
...
def makeInverseIndex(strlist):
    ...
    for(index, document) in enumerate(strlist):
    	for str in document.split():
    		currentValue = dictionary[str] if str in dictionary else set()
    		currentValue.add(index)
    		dictionary[str] = currentValue
    return dictionary
...
def orSearch(inverseIndex, query):
    ...
    queryList = list()
    for s in query:
    	queryList.append(inverseIndex[s])
    return queryList
...
def andSearch(inverseIndex, query):
    ...
    return list(set.intersection(*map(set, orSearch(inverseIndex, query))))
